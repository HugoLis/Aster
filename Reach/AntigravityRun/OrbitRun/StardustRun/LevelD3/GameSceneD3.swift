...
public class GameSceneD3: GameSceneC {
    ...
    override func resetScene () {
        ...
        switch deviceType {
        case 0:
            if let gameScene = GameSceneD3(fileNamed: "GameScene0"){
        ...
        case 2:
            if let gameScene = GameSceneD3(fileNamed: "GameScene2"){
                gameScene.scaleMode = .aspectFill
                self.view?.presentScene(gameScene, transition: reveal)
            }
        default: //case 3
            if let gameScene = GameSceneD3(fileNamed: "GameScene3"){
                gameScene.scaleMode = .aspectFill
                self.view?.presentScene(gameScene, transition: reveal)
            }
        }    }
    ...
    override public func loadNextScene(size: CGSize){
        switch deviceType {
        case 1:
            youWonScene = YouWonSceneD3(size: CGSize(width: 480, height: 640))
        case 2:
            youWonScene = YouWonSceneD3(size: CGSize(width: 480, height: 640))
        default:
            youWonScene = YouWonSceneD3(size: CGSize(width: 450, height: 800))
        }
