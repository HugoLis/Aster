...
public class GameSceneC6: GameSceneC {
    ...
    override func resetScene () {
        ...
        switch deviceType {
        case 0:
            if let gameScene = GameSceneC6(fileNamed: "GameScene0"){
        ...
        case 2:
            if let gameScene = GameSceneC6(fileNamed: "GameScene2"){
                gameScene.scaleMode = .aspectFill
                self.view?.presentScene(gameScene, transition: reveal)
            }
        default: //case 3
            if let gameScene = GameSceneC6(fileNamed: "GameScene3"){
                gameScene.scaleMode = .aspectFill
                self.view?.presentScene(gameScene, transition: reveal)
            }
        }
        ...
        if numberOfStars == 1 {
            ...
            let resetScene = SKAction.run(){
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1, execute: {
                    ...
                    var youWonScene = YouWonSceneC6(size:gameSize)
                    switch deviceType {
                    case 1:
                        youWonScene = YouWonSceneC6(size: CGSize(width: 480, height: 640))
                    case 2:
                        youWonScene = YouWonSceneC6(size: CGSize(width: 450, height: 800))
                    default:
                        youWonScene = YouWonSceneC6(size: CGSize(width: 468.73, height: 1015))
                    }
